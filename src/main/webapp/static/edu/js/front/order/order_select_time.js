...
function selecttime(timeId, dayId, day, obj){
	...
	var _dateTime = new Date(_date.replace(/-/ig,'/'));
	if (currentTime.getTime() > _dateTime.getTime()){
		dialog("约课时间不能早于当前时间", 10, "", "");
		return false;
	} else if (parseInt((_dateTime.getTime() - currentTime.getTime())/(1000*60*60)) < 1) {
		dialog("不能预约当前时间一小时内的时间段", 10, "", "");
		return false;
	}
	if (_flag == -1) {
		...
		$("#selecttime_" + _flag + " span tt").removeClass("c-master").addClass("c-666");
...
function newlyConsult(date, teacherId, flag){
	...
	_flag = flag;
	// 标记被重选的时间段
	$("#selecttime_" + flag + " span tt").removeClass("c-666").addClass("c-master");
	// 跳转到重新预约的时间段所在的周
...
function consultTeacherTime(){
	if (_num != _lessionnum) {
		if (_flag != -1) {
			dialog("您点击了重选约课，请您重新<br/>选择一个时间段上课后再提交！", 9, "", "");
			return;
		}
		dialog("您购买了 " + _lessionnum + " 个课时，<br/>只预约了" + _num + "个时间段。", 9, "", "");
		return;
	}
	// 拼装字符串
	var value = "";
