			...
			if(result.success){
				...
				if(list!=null && list.length>0){
					...
					for(i=0;i<list.length;i++){
						content+='<p><a href="javascript: void(0)" onClick="chooseMajor('+list[i].id+')" title="'+list[i].subjectName+list[i].majorName+'">'+list[i].subjectName+list[i].majorName+'</a></p>';	
					}
...
function save(){
	...
	$.ajax({
		url : baselocation + "/uc/teacher/addClassCourse",
