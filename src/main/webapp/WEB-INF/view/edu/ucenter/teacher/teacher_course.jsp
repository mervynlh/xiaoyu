...
<body>
	<form action="${ctx }/uc/teacher/myclass" method="post" id="searchForm">
				...
				<section class="u-m-c-head">
					<ul id="uTabTitle" class="fl u-m-c-h-txt of">
						<li class=""><a title="课表日历"
							href="${ctx }/uc/teacher/myclass?status=0">课表日历</a></li>
						<li class=""><a title="将要上的课"
							href="${ctx }/uc/teacher/myclass?status=2"">将要上的课(${detailStatus.CLASS })</a>
						</li>
						<li class=""><a title="待确认时间的课"
							href="${ctx }/uc/teacher/myclass?status=1"">待确认时间的课(${detailStatus.TIME })</a>
						</li>
						<li class=""><a title="待确认课酬的课"
							href="${ctx }/uc/teacher/myclass?status=3"">待确认课酬的课(${detailStatus.REWARD })</a>
						</li>
						<li class=""><a title="已上的课"
							href="${ctx }/uc/teacher/myclass?status=6"">已上的课(${detailStatus.OVER+detailStatus.NOTEVALUTE+detailStatus.EVALUTE})</a>
						</li>
					</ul>
					<div class="clear"></div>
				</section>
							...
							<section class="mb50">
								<div class="right-sign-rl">
									<div class="u-s-rl-tit u-s-r-tit">
										<p class="fsize24 c-666 f-fM" id="dateYM"onclick="backToday()"></p>
										<a class="up" href="javascript:void(0)"onclick="changeMonth('up')"></a>
										<a class="down"href="javascript:void(0)" onclick="changeMonth('down')"></a>
									</div>
															...
															<c:choose>
																...
																<c:when test="${status==1 }">
																	<c:if test="${detail.ifConfirm=='STUUNCONFIRM' }">
																		<tt class="fsize12 f-fM c-666 vam">等待学生确认时间</tt>
																	</c:if>
																...
																<c:when test="${status==3 }">
																	<tt class="fsize12 f-fM c-666 vam">等待学生确认课酬</tt>
																</c:when>
																<c:when test="${status==4 or status==5 or status==6}">
													...
													<span class="u-teaname-btn disIb vam">
														<c:choose>
															...
															<c:when test="${detail.status==4}">
																<a href="javascript:void(0)">学生还未评价</a>
																<a href="javascript:summaryDialog(${detail.id } )">课时小结</a>
															</c:when>
