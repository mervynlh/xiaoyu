...
<head>
	...
	<script type="text/javascript" src="${ctx}/static/edu/js/front/course/class_course_update.js"></script>
	<script type="text/javascript">
		$(function(){
			uploadPicLoad('fileupload');
			initKindEditor('courseContent','576px','400px');//添加页面编辑器
			//专业处理
			var subjectId='${subjectId}';
			$("#"+subjectId+"").click();
			$("#major${classCourse.subjectMajor.id}").click();
			//授课类型处理
			if('${courseModel}'==null||'${courseModel}'==''){
				$("#xianxia").click();
				$("#address${classCourse.addressId}").click();
			}else{
				$("#xianshang").click();
			}
			//适合学员处理
			if('${classCourse.suitStudent}'=='top'){
				$("#top").click();
			}else{
				$("#common").click();
			}
			//是否插班处理
			var isJoinClass=$.trim('${classCourse.isJoinClass}');
			$("[name='course.isJoinClass']").each(function(){
				if($.trim($(this).val())==isJoinClass){
					$(this).click();
				}
			});
			//是否完成处理
			var isFinsh=$.trim('${classCourse.isFinsh}');
			$("[name='course.isFinsh']").each(function(){
				if($.trim($(this).val())==isFinsh){
					$(this).click();
				}
			});
			...
			//加载课节表
			$.ajax({
				url : baselocation + "/front/ajax/teacher/kpoints/${classCourse.id}",
				type : "post",
				dataType : "text",
				async : false,
				success : function(result) {
					$("#kpointsList").html(result);
				}
			});
		});
	</script>
...
<body>
<form action="${ctx}/uc/teacher/updateClassCourse" id="submitForm">
<input id="courseId" type="hidden" name="course.id" value="${classCourse.id}"/>
...
<input id="imgs" name="course.classImgs" type="hidden" value="${classCourse.classImgs}"/>
<input id="addressId" type="hidden" name="course.addressId" value="${classCourse.addressId}"/>
<div class="u-m-right">
			<article class="u-m-center"> 
				<section class="u-m-c-wrap"> 
					<section class="u-t-add" id="select-1">
						<ol class="password-body-boy">
							<li>
								...
								<label class="pa-bo-boy-txt">
									<input id="courseName" type="text" name="course.name" maxlength="" value="${classCourse.name}" class="input-1 fl">
									<div class="clear"></div>
								</label>
								<div class="clear"></div>
							</li>
							<li>
								...
								<div class="Evaluation-box-tp">
									<ul class="clearfix e-b-tp-list u-t-jsxq">
										<li>
											<img id="classpic" class="imgss" src="<%=staticImageServer%>${classCourse.classImgs}" alt="">
										</li>
									...
									<div class="selectUI fl">
										<div class="job-select" style="width:150px;z-index:9999;">
											<section class="j-s-defalt">
												...
												<span id="subjectTip">选择学段</span>
											</section>
											<section style="display: none;" class="j-s-option">
												<div class="j-s-o-box">
													<c:forEach items="${teacherSubject}" var="sub">
														<p><a id="${sub.subjectId}" href="javascript: void(0)" onclick="getMajor(${sub.subjectId})" title="${sub.subjectName}">${sub.subjectName}</a></p>
													</c:forEach>
												</div>
											</section>
										</div>
							...
							<li>
								...
								<label class="pa-bo-boy-txt">
									<input id="currentprise" type="text" name="course.currentprice" maxlength="" value="${classCourse.currentprice}" onkeyup="onlyNumber(this,2)" class="input-2 fl">
									<div class="clear"></div>
								</label>
							...
							<li> 
								...
								<label class="pa-bo-boy-txt">
									<input id="courseName" type="text" name="course.peopleNum" maxlength="" value="${classCourse.peopleNum}" class="input-1 fl" onkeyup="this.value=this.value.replace(/\D/g,'')" >
									<div class="clear"></div>
								</label>
								<div class="clear"></div>
							</li>
							<li> 
								...
								<span class="vam pa-bo-boy-txt pa-bo-boy-txt-1">
									<input id="xianxia" type="radio" style="margin: 0 0 0 30px;" onclick="chooseModel(0)" checked="checked"  name="course.courseModel" value="LINEDOWNCOU" > 
									<label class="c-666 fsize14 f-fM" for="">线下授课</label> 
									<input id="xianshang" type="radio" style="margin: 0 0 0 10px;" onclick="chooseModel(1)" name="course.courseModel" value="ONLINECOU" > 
											...
											<section style="display: none;" class="j-s-option">
												<div class="j-s-o-box">
													<c:forEach items="${userAddressList}" var="address">
														<p><a id="address${address.id}" href="javascript: setAddressId(${address.id})" title="">${address.address}</a></p>
													</c:forEach>
												</div>
											</section>
										</div>
							...
							<li>
								...
								<label class="pa-bo-boy-txt pr">
									<input id="common" type="radio" name="course.suitStudent" checked="checked" value="common"/>
									<label class="c-666 fsize14 f-fM" for="">普通</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<input id="top" type="radio" name="course.suitStudent"  value="top"/>
							...
							<li>
								...
								<label class="pa-bo-boy-txt pr">
									<input type="text" name="course.learningTarget" class="input-1 fl" value="${classCourse.learningTarget}"/>
								</label>
								<div class="clear"></div>
							</li>
							<li>
								...
								<label class="pa-bo-boy-txt pr">
									<input type="radio" name="course.isJoinClass" onclick="chooseJoin(0)" checked="checked" value="CANNOT"/>
									<label class="c-666 fsize14 f-fM" for="">不可以插班</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<input type="radio" name="course.isJoinClass"  onclick="chooseJoin(1)"  value="SURE"/>
							...
							<li id="joinPrise" style="display: none;">
								...
								<label class="pa-bo-boy-txt">
									<input type="text" name="course.joinPrise" maxlength="" value="${classCourse.joinPrise}" onkeyup="onlyNumber(this,2)" class="input-2 fl">
									<div class="clear"></div>
								</label>
								<span class="vam pa-bo-boy-tit" style="text-align:left;padding-left:10px;">
									<tt class="fsize14 c-666 f-fM vam">元</tt>
							...
							<li>
								...
								<label class="pa-bo-boy-txt pr">
									<input type="text" name="course.coursePlan" class="input-1 fl" value="${classCourse.coursePlan}"/>
							...
							<li>
								...
								<label class="pa-bo-boy-txt pr">
									<textarea id="courseTitle" maxlength="100" name="course.title" class="vam" style="width:500px;">${classCourse.title}</textarea>
							...
							<li>
								...
								<label class="pa-bo-boy-txt pr">
									<textarea id="courseContent" maxlength="300" name="course.context" class="vam" style="width:800px;">${classCourse.context}</textarea>
						...
						<div id="kpointsList" class="u-t-add-boy mt50">
						</div>
					</section>
				</section>   
			</article>
